...
def create_plant(planta: plantSchema, session: T_Session):
    db_user = session.scalar(select(Plantas).where((Plantas.nome == planta.nome)))
...
def read_plants(session: T_Session, limit: int = 10, offset: int = 0):
    ...
    return {
        "Plants": plan
    }  # A chave precisa ser 'Plants' para ser consistente com o esquema
...
def read_plant(session: T_Session, plant_id: int):
    db_plant = session.scalar(select(Plantas).where(Plantas.id == plant_id))
    if not db_plant:
        raise HTTPException(
...
def update_planta(session: T_Session, planta: plantSchema, plant_id: int):
