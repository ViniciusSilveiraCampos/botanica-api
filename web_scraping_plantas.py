...
from fast_zero.model import Plantas
...
def create_or_update_plant(planta: plantSchema, session: Session):
    db_plant = session.query(Plantas).filter_by(nome=planta.nome).first()
...
url = "https://pt.wikipedia.org/wiki/Lista_de_plantas_do_Brasil"
...
for a_tag in soup.find_all("a"):
    ...
    if text == "Ananas comosus":
        start_collecting = True

    if start_collecting:
        if href and href.startswith("/wiki/"):
            full_url = "https://pt.wikipedia.org" + href
            plantas_links[unquote(text)] = full_url

        if text == "Ximenia americana":
            break


# Função para extrair informações da planta
...
# Coletando os detalhes de cada planta
plantas_detalhes = []

# Obter a sessão
session = next(get_session())

for nome_cientifico, link in plantas_links.items():
    info = extrair_informacoes(nome_cientifico, link)
...
# Opcional: Exibindo o JSON resultante para verificação
json_result = json.dumps(plantas_detalhes, indent=4, ensure_ascii=False)
print(json_result)
